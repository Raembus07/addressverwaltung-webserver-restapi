...
public class HttpclientImpl implements PersonService {
  ...
  @Override
  public void save(@Nonnull PersonDTO personDTO) throws JsonProcessingException {
    HttpRequest request = HttpRequest.newBuilder()
        .uri(URI.create(RestapiConst.REST_URL))
    ...
    try {
      HttpResponse<String> response = client.send(request, HttpResponse.BodyHandlers.ofString());
      if (response.statusCode() == 200) {
        objectMapper.readValue(response.body(), PersonDTO.class);
      } else {
      }
    } catch (IOException | InterruptedException e) {
  ...
  @Override
  public void update(@Nonnull Long id, @Nonnull PersonDTO personDTO) throws JsonProcessingException {
    HttpRequest request = HttpRequest.newBuilder()
        .uri(URI.create(RestapiConst.REST_URL + "/" + id))
        .header("Content-Type", "application/json")
